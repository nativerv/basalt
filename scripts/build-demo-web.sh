...
while test $# -gt 0; do
  case "$1" in
    -h|--help)
      ...
      echo "  --port:    Port where the result is hosted"
      exit 0
      ;;
...
FINAL_WASM_PATH="${OUT_DIR}/${OUT_FILE_NAME}_bg.wasm"
...
wasm-bindgen "${WASM_PATH}" --out-dir "${OUT_DIR}" --out-name ${OUT_FILE_NAME} --no-modules --no-typescript
